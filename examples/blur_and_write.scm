; An example of loading an image, applying a blur filter on it and storing the result in a file
...
; Setup an ImageBlurLink and store the result of its execution under `img-collection key-1`.
(define img-blur (ImageBlur img-node (Number "5")))
(cog-set-value! img-collection key-1 (cog-execute! img-blur))

; Setup an ImageWriteLink with the image to be loaded from `img-collection key-1` and
